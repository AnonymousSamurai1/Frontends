...
function AuthenticationUtils(props) {
  ...
  const handleSecret = async (event)=>{
    ...
    try {
      const res = await fetch('http://localhost:5612/agrobiochem/api/keys/login', {
        method: "POST",
      ...
      <div className="key-main">
        ...
        <form onSubmit={handleSecret} >
          <div className="key-sub">
            <input type="password"name="key" className="key" placeholder="Enter the Key" value={password} onChange={(e)=>setPassword(e.target.value)}/>
          </div>
